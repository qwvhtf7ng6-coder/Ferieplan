  ...
  return (
    <Modal open={!!data} onClose={onClose} title={`${user.name} · ${formatDate(dateKey)}`} className="sm:max-w-md">
      <div className="space-y-4">
        ...
        {requests.map((req) => {
          const dayEntry = req.entries.find((e) => e.date.substring(0, 10) === dateKey);
              ...
              <div className="pt-2 border-t border-border">
                ...
                <div className="flex flex-wrap gap-1">
                  {[...req.entries]
                    ...
                    .map((e) => {
                      const eKey = e.date.substring(0, 10);
                      return (
              ...
              {dept.users.map((emp, empIdx) => (
                <tr key={emp.id} className="group transition-colors"
                  ...
                  {days.map((d) => {
                    ...
                    const approvedEntry = reqs.find((r) => r.status === "APPROVED")?.entries.find((e) => e.date.substring(0, 10) === dk);
...
function MobileCalendarList({
  ...
  const holidayMap = useMemo(() => new Map<string, string>(holidays.map((h) => [h.date.substring(0, 10), h.name])), [holidays]);
