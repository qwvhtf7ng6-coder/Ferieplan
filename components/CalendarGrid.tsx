...
export default function CalendarGrid({
  ...
  const deptCapacity = useMemo(() => {
    // ⚡ Bolt: Optimize deptCapacity calculation from O(D * R * U) to O(D*U + R)
    const outer = new Map<string, Map<string, number>>();

    // Create a fast lookup for user -> department(s) to handle users in multiple departments
    ...
    // Single pass through requests instead of looping per department
    for (const req of requests) {
