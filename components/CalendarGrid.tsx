...
const formatDateKey = (d: string | Date) => typeof d === 'string' ? d.substring(0, 10) : new Date(d).toISOString().slice(0, 10);
  ...
  return (
    <Modal open={!!data} onClose={onClose} title={`${user.name} · ${formatDate(dateKey)}`} className="sm:max-w-md">
      <div className="space-y-4">
        ...
        {requests.map((req) => {
          const dayEntry = req.entries.find((e) => formatDateKey(e.date) === dateKey);
              ...
              <div className="pt-2 border-t border-border">
                ...
                <div className="flex flex-wrap gap-1">
                  {[...req.entries]
                    ...
                    .map((e) => {
                      const eKey = formatDateKey(e.date);
                      return (
              ...
              {dept.users.map((emp, empIdx) => (
                <tr key={emp.id} className="group transition-colors"
                  ...
                  {days.map((d) => {
                    ...
                    const approvedEntry = reqs.find((r) => r.status === "APPROVED")?.entries.find((e) => formatDateKey(e.date) === dk);
...
function MobileCalendarList({
  ...
  const holidayMap = useMemo(() => new Map<string, string>(holidays.map((h) => [formatDateKey(h.date), h.name])), [holidays]);
