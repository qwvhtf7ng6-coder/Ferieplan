...
/**
 * DepartmentsPanel — embeddable variant af DepartmentsClient til Settings-tab.
 * Forskelle:
 *  - Ingen PageHeader (samlesidens header dækker).
 *  - "Ny afdeling"-knap ligger i lokal panel-header.
 *  - Funktionelt identisk med DepartmentsClient.
 */
export default function DepartmentsPanel({
  ...
  function openEdit(d: Department) {
    ...
    setEditForm({
      name: d.name,
      maxConcurrent: String(d.maxConcurrent),
  ...
  return (
    <div>
      ...
      <div className="flex items-end justify-between mb-5">
        <div>
          <p className="text-[15px] font-bold text-text">Afdelinger</p>
          <p className="text-[12px] text-text-muted mt-0.5">{initial.length} afdelinger</p>
        </div>
        <Btn onClick={() => setShowCreate(true)} icon={<Plus size={14} />} size="sm">
          Ny afdeling
        </Btn>
      </div>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                onClick={() =>
                  setForm({
                    ...form,
                    maxConcurrent: String(Math.max(1, parseInt(form.maxConcurrent) - 1)),
                  })
                }
              ...
              <span className="text-[26px] font-extrabold text-primary w-10 text-center">
                {form.maxConcurrent}
              </span>
              <button
                type="button"
                onClick={() =>
                  setForm({
                    ...form,
                    maxConcurrent: String(parseInt(form.maxConcurrent) + 1),
                  })
                }
                className="w-9 h-9 rounded-md border border-border flex items-center justify-center text-text-muted hover:border-primary hover:text-primary transition-colors text-lg font-bold"
              >
          ...
          <div>
            ...
            >
              <div
                ...
                style={{
                  background: form.shiftsEnabled ? "var(--c-primary-muted)" : "var(--c-bg)",
                  color: form.shiftsEnabled ? "var(--c-primary)" : "var(--c-text-subtle)",
                }}
              >
                <Clock size={16} />
              </div>
              <div className="flex-1">
                ...
                <p className="text-[12px] text-text-muted mt-0.5">
                  {form.shiftsEnabled
                    ? "Aktiveret — vagtplan-menuen vises for denne afdeling."
                    : "Slået fra — vagtplan-menuen skjules og siden er utilgængelig."}
                </p>
              </div>
              <Toggle
                checked={form.shiftsEnabled}
                onChange={(v) => setForm({ ...form, shiftsEnabled: v })}
              />
            </div>
          </div>

          <div className="flex gap-2 pt-2">
            <Btn type="submit" disabled={createLoading}>
              {createLoading ? "Opretter..." : "Opret afdeling"}
            </Btn>
      ...
      >
        <div className="space-y-5">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                onClick={() =>
                  setEditForm({
                    ...editForm,
                    maxConcurrent: String(Math.max(1, parseInt(editForm.maxConcurrent) - 1)),
                  })
                }
                className="w-9 h-9 rounded-md border border-border flex items-center justify-center text-text-muted hover:border-primary hover:text-primary transition-colors text-lg font-bold"
              >
              ...
              <button
                ...
                onClick={() =>
                  setEditForm({
                    ...editForm,
                    maxConcurrent: String(parseInt(editForm.maxConcurrent) + 1),
                  })
                }
                className="w-9 h-9 rounded-md border border-border flex items-center justify-center text-text-muted hover:border-primary hover:text-primary transition-colors text-lg font-bold"
              >
          ...
          <div>
            ...
            >
              <div
                className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0"
                style={{
                  background: editForm.shiftsEnabled ? "var(--c-primary-muted)" : "var(--c-bg)",
                  color: editForm.shiftsEnabled ? "var(--c-primary)" : "var(--c-text-subtle)",
                }}
              >
                <Clock size={16} />
              </div>
              ...
              <Toggle
                checked={editForm.shiftsEnabled}
                onChange={(v) => setEditForm({ ...editForm, shiftsEnabled: v })}
              />
            </div>
          </div>
          ...
          <div className="flex gap-2 pt-2">
            <Btn onClick={saveEdit} disabled={editLoading}>
              {editLoading ? "Gemmer..." : "Gem afdeling"}
            </Btn>
            <Btn variant="ghost" onClick={() => setEditDept(null)}>
              Annuller
            </Btn>
          </div>
        </div>
