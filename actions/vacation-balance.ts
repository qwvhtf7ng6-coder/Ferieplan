...
export async function getAllVacationBalances(year?: number) {
  ...
  // ⚡ Bolt Performance Optimization: Replace N+1 queries with a single bulk query
  // Fetches all approved vacation entries for all relevant users in one DB call,
  // then aggregates days in-memory to reduce DB roundtrips from O(N) to O(1).
