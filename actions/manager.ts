...
async function checkCapacity(
  ...
  // ⚡ Bolt: Fetch all relevant entries in a single query to avoid N+1 problem (O(1) instead of O(N))
  const entryDates = entries.map(e => e.date);

  if (entryDates.length === 0) return { exceeded: false };
  ...
  // Group counts by date in memory
