...
async function checkCapacity(
  ...
  // Bolt: Performance optimization - Fix N+1 query bottleneck
  // Instead of counting entries for each date inside a loop, we fetch all relevant entries
  // in a single O(1) query and compute the capacity in-memory.
  ...
  for (const entry of existingEntries) {
    const dStr = entry.date.toISOString().slice(0, 10);
    countByDate.set(dStr, (countByDate.get(dStr) || 0) + 1);
  }
  ...
  for (const entry of entries) {
    ...
    if (count + 1 > dept.maxConcurrent) {
      return { exceeded: true, date: dStr, current: count, max: dept.maxConcurrent };
    }
  }
